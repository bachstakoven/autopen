...
ff_rc = safe_command.run(subprocess.run, ['sudo', 'apt-get', 'install', '-y', 'ffmpeg']).returncode
if ff_rc != 0:
	print ('INSTALLATION FAILED: Failed to install ffmpeg library, trying alternative option: libav-tools')
	lib_rc = safe_command.run(subprocess.run, ['sudo', 'apt-get', 'install', '-y', 'libav-tools']).returncode
	if lib_rc != 0:
		...
		ppa_rc = safe_command.run(subprocess.run, ['sudo', 'add-apt-repository', 'ppa:mc3man/trusty-media']).returncode
		if ppa_rc != 0:
			...
			if up_rc != 0:
				...
				ffm_rc = safe_command.run(subprocess.run, ['sudo', 'apt-get', 'install', 'ffmpeg']).returncode
				if ffm_rc != 0:
					print ('INSTALLATION FAILED: Could not install ffmpeg package. Functionality may be hindered and/or Autopen may not be able to run')
...
libraries_rc = safe_command.run(subprocess.run, ['sudo', 'apt-get', 'install', '-y', 'python3-pip', 'build-essential', 'git', 'python-dev','libsdl2-dev', 'libsdl2-image-dev', 'libsdl2-mixer-dev',\
...
if libraries_rc != 0:
	...
	ch_rc = safe_command.run(subprocess.run, ['sudo', 'pip3', 'install', 'virtualenv', 'setuptools']).returncode
	if ch_rc != 0:
		...
		x_rc = safe_command.run(subprocess.run, ['virtualenv', '--no-site-packages', 'kivyinstall']).returncode
		if x_rc != 0:
			...
			safe_command.run(subprocess.run, ['sudo', 'dpkg-reconfigure', 'dash'])
			print ('VIRTUALENV SUCCESSFUL: Successfully setup kivy isolated environment')
			print ('Entering the virtualenv environment...')
			enter_rc = safe_command.run(subprocess.run, ['source ./kivyinstall/bin/activate'],shell=True).returncode
			if enter_rc != 0:
				...
				cython_rc = safe_command.run(subprocess.run, ['pip3', 'install', 'Cython==0.23']).returncode
				if cython_rc != 0:
					...
					kivy_rc = safe_command.run(subprocess.run, ['pip3', 'install', 'kivy']).returncode
					if kivy_rc != 0:
