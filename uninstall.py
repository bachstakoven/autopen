...
def uninstall(toolname):
	...
	try: 
		if toolname == 'canbus-utils':
			rm_rc = safe_command.run(subprocess.run, ['rm', '-rf', 'canbus-utils']).returncode
		elif toolname == 'Kayak':
			rm_rc = safe_command.run(subprocess.run, ['rm', '-rf', 'Kayak']).returncode
		elif toolname == 'caringcaribou':
			rm_rc = safe_command.run(subprocess.run, ['rm', '-rf', 'caringcaribou']).returncode
		elif toolname == 'c0f':
			rm_rc = safe_command.run(subprocess.run, ['rm', '-rf', 'c0f']).returncode
		elif toolname == 'udsim':
			rm_rc = safe_command.run(subprocess.run, ['rm', '-rf', 'UDSim']).returncode
		elif toolname == 'katoolin':
			rm_rc = safe_command.run(subprocess.run, ['rm', '-rf', 'katoolin']).returncode
		elif toolname == 'bluelog':
			rm_rc = safe_command.run(subprocess.run, ['rm', '-rf', 'Bluelog']).returncode
		elif toolname == 'bluemaho':
			rm_rc = safe_command.run(subprocess.run, ['rm', '-rf', 'bluemaho']).returncode
		elif toolname == 'j1939':
			rm_rc = safe_command.run(subprocess.run, ['rm', '-rf', 'can-utils-j1939']).returncode
		elif toolname == 'canbadger-hw':
			rm_rc = safe_command.run(subprocess.run, ['rm', '-rf', 'CANBadger']).returncode

			#https://github.com/Gutenshit/CANBadger/wiki/Getting-the-board-ready

		elif toolname == 'canbadger-sw':
			rm_rc = safe_command.run(subprocess.run, ['rm', '-rf', 'CANBadger-Server']).returncode

		elif toolname == 'pyobd':
			try:
				rm_rc = safe_command.run(subprocess.run, ['rm', '-rf','pyobd_0.9.3.tar.gz']).returncode
			except:
				pass
			try:
				rm_rc = safe_command.run(subprocess.run, 'rm', '-rf', 'pyobd-0.9.3').returncode
			except:
				pass
		elif toolname == 'o2oo':
			try:
				rm_rc = safe_command.run(subprocess.run, ['rm', '-rf','O2OO-0.9.tgz']).returncode
			except:
				pass
			try:
				rm_rc = safe_command.run(subprocess.run, 'rm', '-rf', 'O2OO-0.9').returncode
			except:
				pass

		elif toolname == 'btscanner':
			rm_rc = safe_command.run(subprocess.run, ['sudo', pack_man, 'purge', '-y','btscanner']).returncode
		elif toolname == 'gnuradio':
			rm_rc = safe_command.run(subprocess.run, ['sudo', pack_man, 'purge','-y', 'gnuradio']).returncode
		elif toolname == 'aircrack-ng':
			rm_rc = safe_command.run(subprocess.run, ['sudo', pack_man, 'purge','-y', 'aircrack-ng']).returncode
		elif toolname == 'gqrx':
			rm_rc = safe_command.run(subprocess.run, ['sudo', pack_man, 'purge', '-y','gqrx']).returncode
		elif toolname == 'can-utils':
			rm_rc = safe_command.run(subprocess.run, ['sudo', pack_man, 'purge','-y', 'can-utils']).returncode
		elif toolname == 'wireshark':
			rm_rc = safe_command.run(subprocess.run, ['sudo', pack_man, 'purge','-y', 'wireshark']).returncode
		elif toolname == 'tshark':
			rm_rc = safe_command.run(subprocess.run, ['sudo', pack_man, 'purge','-y', 'tshark']).returncode
